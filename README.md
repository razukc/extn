...
5. 💾 A persistent profile is created in `.dev-profile/` (gitignored)
...
**Problem:** Extension state is lost or profile is corrupted

**Solution:**
- Delete the `.dev-profile/` directory and restart `npm run dev`
- Check that `.dev-profile/` is in your `.gitignore`
